...
export default function CeoMessageNewPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setLoading(true);
    try {
      const result = await api.createCeoMessage({
        title,
        description,
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      <header className="bg-white shadow-sm border-b border-gray-200">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <h1 className="text-xl font-bold text-gray-900">Nouveau message CEO</h1>
          <button
            onClick={() => router.push("/admin/ceomessage")}
      ...
      <main className="max-w-lg mx-auto px-4 py-12">
        <div className="bg-white shadow-sm border border-gray-200 rounded-xl p-6">
          <div className="space-y-1 mb-6">
            <h2 className="text-lg font-semibold text-gray-900">Créer un message CEO</h2>
            <p className="text-sm text-gray-500">Ce message s&apos;affiche dans la section CEO de la page d&apos;accueil</p>
          </div>
