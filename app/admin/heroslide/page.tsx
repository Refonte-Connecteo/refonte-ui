...
export default function HeroSlideListPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const result = await api.getAllHeroSlides();
      ...
      <header className="bg-white shadow-sm border-b border-gray-200">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <h1 className="text-xl font-bold text-gray-900">Slides Hero</h1>
          </div>
          <div className="flex items-center gap-4">
            ...
            <button
              onClick={() => router.push("/admin/heroslide/new")}
        ...
        <div className="bg-white shadow-sm border border-gray-200 rounded-xl overflow-hidden">
                ...
                <tr key={slide.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-5 py-3.5">
                    <span className="inline-flex items-center gap-1 text-blue-600 text-xs">
                      ...
                      {slide.image_url.length > 40 ? slide.image_url.slice(0, 40) + "..." : slide.image_url}
                    </span>
                  </td>
                  <td className="px-5 py-3.5 text-gray-900 font-medium">{slide.title || <span className="text-gray-300">—</span>}</td>
                  <td className="px-5 py-3.5 text-gray-700">{slide.position}</td>
                  <td className="px-5 py-3.5">
                    <button
                    ...
                    >
                      <span className={`w-1.5 h-1.5 rounded-full ${slide.is_active ? "bg-green-500" : "bg-gray-400"}`} />
                  ...
                  <td className="px-5 py-3.5 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button
                        onClick={() => router.push(`/admin/heroslide/${slide.id}/edit`)}
          ...
          {slides.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-gray-400 text-sm">Aucun slide</p>
            </div>
          )}
        </div>
      </main>
