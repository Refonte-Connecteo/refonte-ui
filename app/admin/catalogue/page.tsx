...
export default function CatalogueListPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const result = await api.getAllCatalogues();
  ...
  const handleDelete = async (id: number) => {
    if (!confirm("Voulez-vous vraiment supprimer ce catalogue ?")) return;
    try {
      ...
      <header className="bg-white shadow-sm border-b border-gray-200">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <h1 className="text-xl font-bold text-gray-900">Catalogues</h1>
          </div>
        ...
        <div className="bg-white shadow-sm border border-gray-200 rounded-xl overflow-hidden">
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                ...
                <th className="text-left px-5 py-3.5 font-semibold text-gray-600 text-xs uppercase tracking-wider">Fiche</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {catalogues.map((cat) => (
                <tr key={cat.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-5 py-3.5 text-gray-900 font-medium">{cat.title}</td>
                  ...
                  <td className="px-5 py-3.5 text-gray-500 text-xs">
                    {new Date(cat.uploaded_at).toLocaleDateString("fr-FR", {
                  ...
                  <td className="px-5 py-3.5 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button
                        onClick={() => router.push(`/admin/catalogue/${cat.id}/edit`)}
          ...
          {catalogues.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-gray-400 text-sm">Aucun catalogue</p>
            </div>
          )}
        </div>
      </main>
